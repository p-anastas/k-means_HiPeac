                  ...
                  int  *numCoords)     /* no. coordinates */
{
    ...
    if (isBinaryFile) {  /* input file is in raw binary format -------------*/
        ...
        assert(objects != NULL);
	#pragma omp parallel for private(i) shared(objects) schedule(static)
	for (i=0; i<(*numObjs); i++) {
		objects[i] = (float*)  calloc( len , sizeof(float));
        	numBytesRead = read(infile, objects[i], len*sizeof(float));
