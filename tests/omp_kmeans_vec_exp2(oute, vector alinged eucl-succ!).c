...
{	
	...
	#pragma vector aligned
    	for (i=0; i<numdims; i++) ans +=  (coord1[i]-coord2[i])*(coord1[i]-coord2[i]);
	return ans;
}
               ...
               float **clusters)          /* out: [numClusters][numCoords] */
{
	...
	#pragma omp parallel for private(i,j) shared(clusters, newClusters, newClusterSize) firstprivate(numObjs,numClusters,numCoords) schedule(static)
        for (i=0; i<numClusters; i++) {
	    #pragma vector aligned
            for (j=0; j<numCoords; j++) {
